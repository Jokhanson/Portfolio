      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 w-[280px] z-[100] bg-[#0A0A0F]/60 backdrop-blur-2xl border-l border-white/[0.08] shadow-[-8px_0_30px_rgba(0,0,0,0.5)] flex flex-col md:hidden"
          >
            ...
            <div className="flex-1 flex flex-col gap-1 p-6">
              {navItems.map(([key, id]) => (
                <button
                  ...
                  className="w-full text-left px-4 py-3 rounded-xl text-base text-foreground hover:text-[#62B2FE] hover:bg-white/[0.12] transition-colors capitalize font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#62B2FE]"
                >
                  {t(`nav.${key}`)}
                </button>
              ))}
